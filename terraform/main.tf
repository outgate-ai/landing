...
variable "domain" {
  description = "Custom domain for the landing site"
  default     = "dev.outgate.ai"
}

################################################################################
# Infrastructure remote state — reads cert ARNs from infrastructure repo
################################################################################
...
################################################################################
# S3 Bucket — CLI releases (served at /download/*)
################################################################################
...
################################################################################
# CloudFront Function — serve prerendered HTML to bots/crawlers
################################################################################
...
function handler(event) {
  ...
  // Serve prerendered HTML to non-browser user agents.
  // Real browsers always include a rendering engine token like
  // Chrome/, Firefox/, Safari/, Edge/, or Opera/ in their UA.
  // Anything without one is a bot, crawler, or fetch tool.
  var isBrowser = /chrome\/|firefox\/|safari\/|edg\/|opera\/|opr\//i.test(ua);

  if (!isBrowser) {
    // Map legal pages to their prerendered files
...
resource "aws_cloudfront_distribution" "landing" {
  ...
  # /download/* → CLI releases bucket (strip /download prefix via origin path not needed;
  # S3 key includes the /download prefix, so we store as download/latest/gw-darwin-arm64 etc.)
  ...
  # SPA: serve index.html for missing paths
...
resource "aws_s3_bucket_policy" "landing" {
  ...
  policy = jsonencode({
    ...
    Statement = [{
      ...
      Condition = {
        StringEquals = { "AWS:SourceArn" = aws_cloudfront_distribution.landing.arn }
      }
    }]
  })
}
...
output "cloudfront_domain" {
  ...
  description = "CloudFront domain — point your DNS CNAME to this value"
}

output "cli_releases_bucket" {
  value       = aws_s3_bucket.cli_releases.bucket
  description = "S3 bucket for CLI release binaries"
}
